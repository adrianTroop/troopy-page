...
describe("LotteryContract", () => {
    ...
    describe("Deposit Tokens and check balance", () =>{
        ...
        it("Returns a random winner", async ()=>{
            transaction = await lotteryContract.connect(user2).deposit({ value: amount })
            result = await transaction.wait()
            transaction = await lotteryContract.connect(user3).deposit({ value: amount })
            result = await transaction.wait()
            const winner = await lotteryContract.pickWinner();
            result = await transaction.wait()
            const depositersList = await lotteryContract.getDepositersList();
            const depositerAddresses = depositersList.map(depositer => depositer.userAddress);
            expect(depositerAddresses).to.include(winner);
        })
        }) 
    })
